...
def fbx_data_armature_elements(root, arm_obj, scene_data):
    ...
    if deformer is not None:
        for me, (skin_key, ob_obj, clusters) in deformer.items():
            ...
            # Create clusters only for bones that have a corresponding vertex group.
            for bo_obj, clstr_key in clusters.items():
                bo = bo_obj.bdata
                # Check if the bone has an associated vertex group.
                vg_idx = bo_vg_idx.get(bo.name, None)
                
                # If there is no vertex group associated with this bone, skip creating a cluster.
                if vg_idx is None or not vgroups[vg_idx]:
                    continue
                
                # Find the vertices influenced by this bone and their weights.
                indices, weights = zip(*vgroups[vg_idx].items())
                ...
                # Add vertex indices and weights to the cluster.
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    for ob_obj in objects:
        ...
        if settings.armature_nodetype == 'REMOVE_GHOST':
            if ob_obj.type == 'ARMATURE':
                continue
...
def save_single(operator, scene, depsgraph, filepath="",
                ...
                armature_nodetype='REMOVE_GHOST', # [Alessandro]
                colors_type='SRGB',
